...
function Detail2() {
  ...
  // }
  const checkError=()=>{
    if (input.author&&input.title&&input.numOfPage&&input.amount&&input.decription&&input.category&&input.date){
      setError(null)
    }
    else {
      setError("Không được để trống thông tin sách!")
    }
  }
  const handleAdd=async (input)=>{
    // input.amount=Number(input.amount)
    // input.numOfPage=Number(input.numOfPage)
